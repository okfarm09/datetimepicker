 ...
 */
const dateTimePicker = (callbackFunction) => {
	const easing = {
	...
	const hours = new Array(24).fill(1).map((v, i) => {
		return {
			value: i + 1,
			text: i + 1 + '시'
		}
	...
	yearSelector = new IosSelector({
		...
		onChange: (selected) => {
			...
			callbackFunction(yearSelector.value, monthSelector.value, daySelector.value, hourSelector.value, minuteSelector.value);
		}
	});
	...
	setTimeout(function () {
		...
		minuteSelector.select(now.getMinutes());
	});
}